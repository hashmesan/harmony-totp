...
describe("SmartVault test", () => {
    ...
    it.only("creates an account", async () => {
        var client = new SmartVault(config.CONFIG["development"]);
        var data = await client.create("hashmesan103.crazy.one");
        ...
        var methodData = RelayerClient.getContract().methods.multiCall([{to: account.address, value: Web3.utils.toWei("0.0123"), data: "0x"}]).encodeABI();
        ...
        // var methodData = RelayerClient.getContract().methods.multiCall([{to: account.address, value: Web3.utils.toWei("1000"), data: "0x"}]).encodeABI();
