...
function normalize(a) {
  return new web3utils.BN(a).toArrayLike(Buffer, "le", 32);
}
function toHex(a) {
  return web3utils.toHex(Buffer.from(a));
}
function h32(a) {
  return new Keccak(256)
    .update(new web3utils.BN(a).toArrayLike(Buffer, "le", 32))
    .digest();
}
function padNumber(x) {
  return web3utils.padRight(x, 64);
}
function getTOTP(secret, counter, duration) {
  return totp(secret, { period: duration, counter: counter });
}
const NUMOFTOKENS = 5;
...
function generateWallet(secret, depth, duration, timeOffset, fProgress) {
  ...
  for (var i = 0; i < Math.pow(2, depth); i++) {
    leafs.push(
      h16(
        padNumber(web3utils.toHex(getTOTP(secret, startCounter + i, duration)))
      )
    );
...
function verifyProof(proof, rootHash) {
  ...
  proofCopy[0] = web3utils.soliditySha3({
    t: "bytes32",
    v: proofCopy[0],
    encoding: "hex",
  });
...
function createHOTP(secret, depth, progress) {
  var root_arr = [];
  const leaves_arr = new Uint8Array(
    new ArrayBuffer(1 + Math.pow(2, depth) * 5 * 32)
  );
...
async function signRecoveryOffchain(
  signers,
  rootHash,
  merkelHeight,
  timePeriod,
...
async function signCreateTx(
  ...
  const messageHash = getCreateHash(
    owner,
    rootHash,
    merkleHeight,
    drainAddr,
...
async function signOffchain2(
  ...
  console.log("signing off chai");
  const messageHash = getMessageHash2(
    from,
    value,
    data,
    chainId,
