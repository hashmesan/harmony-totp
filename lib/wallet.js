...
function normalize(a) { return new web3utils.BN(a).toBuffer("le", 32) }
function toHex(a) { return web3utils.toHex(Buffer.from(a)) }
function h32(a) { return new Keccak(256).update(new web3utils.BN(a).toBuffer("le", 32)).digest(); }
...
function getLeavesAndRoot(secret, offset, depth) {
    ...
    for ( var i=0; i < Math.pow(2, depth); i++) {
      ...
      //console.log(token, toHex(h32(token)));
...
function verifyProof(proof, rootHash) {
  proof[0] = web3utils.soliditySha3({ t: 'bytes32', v: proof[0], encoding: 'hex'})
  var reduced = merkleHex.reduceAuthPath(proof);
