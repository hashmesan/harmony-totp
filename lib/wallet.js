...
function getLeavesAndRoot(secret, offset, depth) {
    ...
    for ( var i=0; i < Math.pow(2, depth); i++) {
      ...
      leaves.push(h32(padNumber(web3utils.toHex(token))));
    }
  ...
  for(var i=0;i<leavesSet.length;i++){
    for(var j=0;j<leavesSet[i].length; j++) {
      if(hash==leavesSet[i][j]) {
        return { leaves: leavesSet[i], counter: j };
      }
    }
  }
  return {leaves: null, counter: 0};
}

function getProofWithOTP(currentOTPS, leavesSet) {  
  var token = currentOTPS.join("");

  var {leaves, counter} = findInLeavesSet(h32(padNumber(web3utils.toHex(token))), leavesSet);
  console.log("Found in counter=", counter);

  if(leaves == null) {
    throw "Cannot build proof: Token cannot be found"
  }
  var proof = merkle.getProof(leaves, counter, padNumber(web3utils.toHex(token)))
  return {token, proof};
}

function createHOTP(secret, depth) {
