...
class SmartVault {
    ...
    async create(name, duration, dailyLimit, drainAddress) {
        ...
        this.walletData = {name, dailyLimit, drainAddress, rentPrice: available.rentPrice, createFee: this.relayClient.createFee};
        this.getOTPSecret();
     ...
     */
    async submitWallet(fStatus, skipIPFS) {
        await this.relayClient.getFees();
    ...
    saveWalletLocal(dir) {
        dir = (dir || ".smartvault/") + this.config.ENV + "/";
