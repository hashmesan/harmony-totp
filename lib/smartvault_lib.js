...
class SmartVault {
    ...
    async setup() {
        ...
        switch (this.config.ENV) {
            ...
            case "development": chainId = await this.harmonyClient.web3.eth.getChainId(); break;
            case "testnet0": chainId = 2; break;
            case "testnet3": chainId = 2; break;
            case "mainnet0": chainId = 1; break;
        } 
        ...
        if ("walletAddress" in this.walletData) {
            try {
                ...
                this.relayClient.setCodeVersion(2);
            }
        } else {
            // new wallet can assume it's using newest
            this.relayClient.setCodeVersion(2);
    ...
    async create(name, duration, dailyLimit, drainAddress) {
        ...
        // TODO: race condition on name reservation could be a problem
        // need redis or memcache to hold onto names for x hours.
    ...
    async submitWallet(fStatus, skipIPFS, height) {        
        ...
        if (!skipIPFS) {
            ...
            const tx = await this.relayClient.storeHashes(this.walletData.walletAddress, this.walletData.root_arr, base64.encode(this.walletData.leaves_arr));
            hashId = tx.data.result.Hash;
            fStatus && fStatus(`Uploaded Hashes to IPFS Hash=${hashId} (${tx.data.result.Size} Bytes)`);            
        }
        ...
        const nameParts = this.walletData.name.split(".");
        await this.relayClient.submitWallet(
                    [nameParts[0], nameParts[1], hashId],
