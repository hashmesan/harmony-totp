...
const default_depth = 6; //TODO: reset to 12
...
class SmartVault {
  ...
  async setup() {
    ...
    switch (this.config.ENV) {
      //KINDA lame the chain id does not reflect what it is supposed to be.
      case "development":
        chainId = await this.harmonyClient.web3.eth.getChainId();
        break;
      case "testnet0":
        chainId = 2;
        break;
    ...
    this.relayClient.setOptions({ chainId });

    // wallet version?
    if ("walletAddress" in this.walletData) {
      const info = await this.harmonyClient.getSmartVaultInfo(
      ...
      this.relayClient.setCodeVersion(info.codeVersion);
    } else {
      // new wallet can assume it's using newest
      this.relayClient.setCodeVersion(1);
    }
  }
  /**
   *
  ...
  async create(
    ...
    this.walletData = {
      ...
      dailyLimit,
      drainAddress,
      rentPrice: utils.roundWei(available.rentPrice),
    ...
    const res = await this.relayClient.getDepositAddress(
  ...
  generateNewOwnerKey() {
    ...
    this.walletData = Object.assign(this.walletData, {
  ...
  generateHashes(height, progress) {
    var mywallet = createHOTP(
      this.walletData.secret,
  ...
  getDepositInfo() {
    return {
      ...
      totalFee: new web3utils.BN(this.walletData.createFee)
        .add(new web3utils.BN(this.walletData.rentPrice))
        .toString(),
    };
  }
  ...
  async submitWallet(fStatus, skipIPFS, height) {
    ...
    if (this.walletData.root_arr == undefined) {
      throw new Error(
        "Must generate leaves first; try calling generateHashes()"
      );
    }
    ...
    await this.relayClient.submitWallet(
      ...
      this.walletData.salt,
      this.walletData.password,
      this.walletData.email,
      this.walletData.countryOfResidence,
      resolver
    );

    fStatus && fStatus("submitWallet complete, waiting for setup");

    await this.setup();

    fStatus && fStatus("setup complete, waiting for IPFS");

    //TODO: Best effort try here
    if (!skipIPFS) {
      // upload IPFS
      fStatus && fStatus("Uploading hashes to IPFS... ");
      const tx = await this.relayClient.storeHashes(
        this.walletData.walletAddress,
        base64.encode(this.walletData.leaves_arr)
      );
      fStatus &&
        fStatus(
          `Uploaded Hashes to IPFS Hash=${tx.data.result.Hash} (${tx.data.result.Size} Bytes)`
        );
  ...
  // we'll require users to upgrade contract with newest startRecoverCommit to recover
  // so we don't have to build backward support
  async recoverWallet(name, codes, fStatus) {
    ...
    fStatus &&
      fStatus(
        `Wallet owned by ${info.owner} counter=${info.counter} hashStorageId=${info.hashStorageID}`
      );

    this.generateNewOwnerKey();
    fStatus &&
      fStatus("Generated new owner key=" + this.walletData.ownerAddress);

    // load hashes
    const hashData = await this.relayClient.getHashes(
      this.walletData.walletAddress
    );
    this.walletData.leaves_arr = base64.decode(hashData.data.result);

    fStatus &&
      fStatus("Loaded merkle hashes len=" + this.walletData.leaves_arr.length);

    // get proof
    var { token, proof, counter, index } = getProofWithOTP(
      codes,
    ...
    // submit commit
    var secretHash = web3utils.soliditySha3(proof[0]);
    var commitHash = web3utils.soliditySha3(
      merkleHex.concat(this.walletData.ownerAddress, proof[0])
    );

    var tx = await this.relayClient.startRecoverCommit(
      this.walletData.walletAddress,
      secretHash,
      commitHash,
      this.config.gasPrice,
    ...
    // submit reveal
    tx = await this.relayClient.startRecoverReveal(
      this.walletData.walletAddress,
      this.walletData.ownerAddress,
      proof,
      this.config.gasPrice,
  ...
  loadFromWalletData(data) {
    ...
    if (data.ownerSecret) {
      ...
      this.ownerAccount = new Web3EthAccounts().privateKeyToAccount(
        data.ownerSecret
      );
    }
  }
  ...
  saveWalletLocal(dir) {
    ...
    var filename =
      dir +
      `.smartvault-${toBech32(walletData.walletAddress)}-${walletData.name}`;
  ...
  listWallets(dir) {
    ...
    var addresses = matches.map((e) => {
      return e.split("-")[1];
    });
    var names = matches.map((e) => {
      return e.split("-")[2];
    });
  ...
  async submitTransaction(methodData, gasPrice, gasLimit) {
    ...
    return await this.relayClient.submitMetaTx(
      this.walletData.walletAddress,
      methodData,
      gasPrice,
      gasLimit,
  ...
  async getTransactions() {
    return await this.harmonyClient.getTransactionsByAccount(
      this.walletData.walletAddress
    );
  }

  async getSmartVaultInfo() {
    return await this.harmonyClient.getSmartVaultInfo(
      this.walletData.walletAddress
    );
  }

  async getUserData() {
    return await this.harmonyClient.getUserData(this.walletData.walletAddress);
  }

  // Return all holding ERC20 tokens address
  async getHoldingTokens() {
    let holdingTokens = [];

    const transactions = await this.harmonyClient.getErc20Transactions(
      this.walletData.walletAddress
    );
    ...
    transactions.forEach((transaction) => {
      if (
        transaction.to != this.walletData.walletAddress.toLowerCase() &&
        transaction.hasOwnProperty("logs") &&
        Object.keys(transaction.logs).length
      ) {
        transaction.logs.forEach((log) => {
          holdingTokens.push(log.address);
        });
      }
    });
    const holdingTokensUnique = Array.from(new Set(holdingTokens));

    return holdingTokensUnique;
  }

  async getTokenInfo(tokenAddress) {
    const erc20Balance = await this.harmonyClient.getErc20Balance(
      tokenAddress,
      this.walletData.walletAddress
    );

    const { name, decimals, symbol } = await this.harmonyClient.getERC20Info(
      tokenAddress
    );
    const balanceForUser = erc20Balance / 10 ** decimals;
  ...
  async addGuardians(guardians) {
    ...
    this.walletData = { ...this.walletData, guardians: guardians };
    return this.walletData;
  }

  async addFriends(friends) {
    this.walletData = { ...this.walletData, friends: friends };
  }
}
