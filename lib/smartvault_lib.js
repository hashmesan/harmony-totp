...
class SmartVault {
  ...
  async setup() {
    ...
    if ("walletAddress" in this.walletData) {
      const info = await this.harmonyClient.getSmartVaultInfo(this.walletData.walletAddress);
      console.log("CodeVersion=", info.codeVersion, chainId, this.walletData.walletAddress);
   ...
   */
  async create(name, duration, dailyLimit, drainAddress, password, email, countryOfResidence) {
    await this.relayClient.getFees();
    ...
    this.walletData = { name, dailyLimit, drainAddress, rentPrice: utils.roundWei(available.rentPrice), createFee: this.relayClient.createFee, password, email, countryOfResidence };
    this.getOTPSecret();
  ...
  async submitWallet(fStatus, skipIPFS, height) {
    ...
    if (!skipIPFS) {
      ...
      // update Contract
      const setIdTx = await this.relayClient.setHashStorageId(this.walletData.walletAddress, tx.data.result.Hash, this.config.gasPrice, this.config.gasLimit, this.ownerAccount);
  ...
  async recoverWallet(name, codes, fStatus) {
    ...
    // get proof
    var { token, proof, counter, index } = getProofWithOTP(codes, this.walletData.leaves_arr);

    fStatus && fStatus(`Generated proof(${proof.length}) counter=${counter}`);

    // submit commit
    var secretHash = web3utils.soliditySha3(proof[0]);
    var commitHash = web3utils.soliditySha3(merkleHex.concat(this.walletData.ownerAddress, proof[0]));

    var tx = await this.relayClient.startRecoverCommit(this.walletData.walletAddress, secretHash, commitHash, this.config.gasPrice, this.config.gasLimit);

    fStatus && fStatus(`Submitted recovery commitHash=${commitHash}`);

    // submit reveal
    tx = await this.relayClient.startRecoverReveal(this.walletData.walletAddress, this.walletData.ownerAddress, proof, this.config.gasPrice, this.config.gasLimit);
  ...
  async getHoldingTokens() {
    ...
    const transactions = await this.harmonyClient.getErc20Transactions(this.walletData.walletAddress);
    ...
    transactions.forEach((transaction) => {
      if (transaction.hasOwnProperty("logs") && Object.keys(transaction.logs).length) {
  ...
  async getTokenInfo(tokenAddress) {
    const erc20Balance = await this.harmonyClient.getErc20Balance(tokenAddress, this.walletData.walletAddress);

    const { name, decimals, symbol } = await this.harmonyClient.getERC20Info(tokenAddress);
