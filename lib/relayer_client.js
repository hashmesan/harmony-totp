...
class RelayerClient { 
    ...
    async submitMetaTx(from, methodData, gasPrice, gasLimit, feeAccount, signingAccount) {
    ...
    async startRecoverCommit(from, commitHash, gasPrice, gasLimit, ownerAccount) {
        const methodData = contract.methods.startRecoverCommit(commitHash).encodeABI();
        return this.submitMetaTx(from, methodData, gasPrice, gasLimit, this.refundAddress, ownerAccount);
    }

    async startRecoverReveal(from, owner, proof, gasPrice, gasLimit, ownerAccount) {
    ...
    async submitWallet(name, ownerAddress, root_arr, merkleHeight, drainAddr, dailyLimit, salt) {
        return axios.post(this.url, {
