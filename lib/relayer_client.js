...
function RelayException(message, data) {
...
class RelayerClient { 
        ...
        try {
            ...
            if (res.status == 200) {
                ...
                if (receipt.name == "Error") {
                    throw new RelayException("GenericError", res.data.result.tx);
                }
                ...
                if(receipt.status == true) {
                    ...
                    if(success) {
                        ...
                        throw new RelayException(error, res.data.result.tx);
                    }
                }
                throw new RelayException("Blockchain error", res.data.result.tx);
            }
        } catch(e) {
            if(e.response) {
    ...
    async startRecoverCommit(from, secretHash, commitHash, gasPrice, gasLimit) {
        if(this.codeVersion == 0) {
            ...
            var methodData = iface.encodeFunctionData("startRecoverCommit",[commitHash]);
