...
class Transactions {
  ...
  async getWalletFactory() {
    ...
    const address =
      walletFactoryArtifacts.networks[this.config.network_id].address;
  ...
  async getFactoryInfo() {
    ...
    const releasePath =
      __dirname + "/../../releases/" + this.env + "/" + implementation + ".txt";
  ...
  async getContractCreated() {
    ...
    for (var i = 0; i < factories.length; i++) {
      ...
      res[address] = created.map((e) => {
        return { wallet: e.wallet, domain: e.domain };
      });
    }
