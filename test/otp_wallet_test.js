...
contract("OTPWallet", (accounts) => {
  ...
  it("should transfer with direct signed requests", async () => {
    ...
    await wallet.makeTransfer(
      tmpWallet.address,
      web3.utils.toWei("0.001234", "ether")
    );
    ...
    assert.equal(
      newBalance,
      web3.utils.toWei(".001234", "ether"),
      "withdraw amount is correct"
    );
  ...
  it("should transfer with meta request from relayer", async () => {
    ...
    const methodData0 = wallet.contract.methods
      .setHashStorageId("BLAH")
      .encodeABI();
    ...
    const methodData = wallet.contract.methods
      .makeTransfer(tmpWallet.address, web3.utils.toWei("0.0012345", "ether"))
      .encodeABI();
    ...
    assert.equal(
      newBalance,
      web3.utils.toWei(".0012345", "ether"),
      "withdraw amount is correct"
    );
  });
  ...
  it("should transfer between smartvaults", async () => {
    ...
    const methodData = wallet.contract.methods
      .makeTransfer(wallet2.wallet.address, web3.utils.toWei("0.1234", "ether"))
      .encodeABI();
    const nonce = await commons.getNonceForRelay();
    const gasLimit = 100000;
    ...
    await truffleAssert.reverts(
      tx,
      "RM: Invalid signatures -- Reason given: RM: Invalid signatures"
    );
  });
  ...
  it("should transfer between smartvaults via factory", async () => {
    ...
    var tx = await wallet.executeMetaTx(
      methodData,
      sigs,
      nonce,
      0,
