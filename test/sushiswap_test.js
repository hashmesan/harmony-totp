...
<<<<<<< HEAD
    const result = await sushi.swapExactETHForTokens(
      amountOutMin,
      path,
      myAddress,
      deadline,
      { from: myAddress, value: amountSendWei }
    );
    console.log(result.receipt);
    expect(result.receipt).not.to.be.empty;
=======
...
>>>>>>> 81ef249124895789322e3a43da8df8879c64c3e1
  ...
  // TODO: check pool size
  it("should have liquidity in the pool", async () => {
    const liq0 = await sushi.name();
    console.log("liq0: ", liq0);
  });
});
