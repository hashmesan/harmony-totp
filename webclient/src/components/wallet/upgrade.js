...
class UpgradePage extends Component {
  ...
  componentDidMount() {
    var self = this;
    Promise.all([
      this.context.smartvault.relayClient.getFactoryInfo(),
      this.context.smartvault.getSmartVaultInfo(),
    ]).then((results) => {
      console.log(results);
      ...
      self.setState({
        ...
        gasPrice: web3utils.fromWei(
          self.context.smartvault.config.gasPrice + "",
          "gwei"
        ),
        gasFee: web3utils
          .toBN(self.context.smartvault.config.gasLimit)
          .mul(web3utils.toBN(self.context.smartvault.config.gasPrice)),
  ...
  transfer(e) {
    ...
    var methodData = RelayerClient.getContract()
      .methods.upgradeMasterCopy(this.state.factoryInfo.implementation)
      .encodeABI();
                ...
                <div className="form-group">
                  <div>
                    ...
                    {this.state.contractInfo &&
                      this.state.contractInfo.masterCopy}
                  </div>
                  <div>
                    Latest Version:{" "}
                    {this.state.factoryInfo &&
                      this.state.factoryInfo.implementation}
                  </div>
                </div>
                <div className="form-group row">
                  <pre className="col-12">
                    {this.state.factoryInfo &&
                      this.state.factoryInfo.releaseNotes}
                  </pre>
                </div>
                {this.state.error && (
                ...
                <div className="form-group row mt-4">
                  {this.state.different ? (
                    ...
                    <div className="col-sm-12 text-center">
                      <b>Your contract is using the latest version</b>
                    </div>
                  )}
                </div>
              </form>
