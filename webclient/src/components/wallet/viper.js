...
class Stats extends Component {
    ...
    this.context.smartvault.harmonyClient.getNetworkId().then((networkId) => {
      ...
      axios.get(default_list).then((e) => {
        ...
        this.setState(
          {
            tokens: tokens,
            to: tokens.length > 0 ? tokens[0] : { symbol: "ETH" },
          },
          () => {
  ...
  swap(e) {
    e.preventDefault();
    if (!this.state.fromAmount || isNaN(parseFloat(this.state.fromAmount)))
      return;
    ...
    swapToken(
      ...
      web3utils.toWei(this.state.toAmount)
    )
      .then((res) => {
        console.log("res", res);
        this.updateTokenAmount("from");
        ...
        // add to ERC20
        this.context.smartvault.walletData.erc20 =
          this.context.smartvault.walletData.erc20 || [];
        if (
          this.context.smartvault.walletData.erc20.findIndex(
            (e) => e.contractAddress == this.state.to.address
          ) == -1
        ) {
  ...
  updateFromAmount() {
    ...
    getBestAmountOut(
      ...
      this.setState({
        toAmount: Number(web3utils.fromWei(amount)).toFixed(6),
        toAmountRaw: amount,
      });
    });
    ...
    if (
      ...
      Number(web3utils.fromWei(this.state.from_amount || "0"))
    ) {
  ...
  updateToAmount() {
    if (this.state.toAmount == "" || isNaN(parseFloat(this.state.toAmount)))
      return;
    ...
    getBestAmountIn(
      ...
      this.setState({
        fromAmount: Number(web3utils.fromWei(amount)).toPrecision(6),
      });
    });
  }
        ...
        <div className="card-body text-secondary p-5">
          <form>
            <div className="form-group">
              ...
              <span className="float-right p-2">
                ...
                {this.state.from_amount &&
                  Number(
                    web3utils.fromWei(this.state.from_amount)
                  ).toFixedNoRounding(4)}
              </span>
              <div className="input-group">
                <input
                  ...
                  onChange={this.changeFromAmount.bind(this)}
                />
                <div className="input-group-append">
                  <button
                    className="btn btn-outline-secondary"
                    type="button"
                    onClick={this.showToken.bind(this, "from")}
                  >
                    <img
                      src={this.state.from.logoURI}
            ...
            <div className="form-group text-center">
              <a href="#" onClick={this.reverseToken.bind(this)}>
            ...
            <div className="form-group">
              ...
              <span className="float-right p-2">
                ...
                {this.state.to_amount &&
                  Number(
                    web3utils.fromWei(this.state.to_amount)
                  ).toFixedNoRounding(4)}
              </span>
              <div className="input-group">
                <input
                  ...
                  onChange={this.changeToAmount.bind(this)}
                />
                <div className="input-group-append">
                  <button
                    className="btn btn-outline-secondary"
                    type="button"
                    onClick={this.showToken.bind(this, "to")}
                  >
                    <img
                      src={this.state.to.logoURI}
                      className="float-left mr-2"
                      style={{ width: 25 }}
                    />
            ...
            <div className="form-group row mt-4">
              <div className="col-sm-12 text-center">
                {this.state.insufficient && (
                  <button className="btn btn-primary btn-lg w-50" disabled>
                    INSUFFICIENT {this.state.from.symbol} balance
                  </button>
                )}
                {!this.state.submitting && !this.state.insufficient && (
                  <button
              ...
              <div className="col-sm-12 text-center">
                {this.state.error && (
                  <div className="alert alert-danger w-100" role="alert">
                    An error occured while swapping{" "}
          ...
          <TokenSelect
            chooseToken={this.chooseToken.bind(this)}
