  ...
  if (!data.function) {
    return (
      <div className="mb-2">
        <div>
          <div>
            <b>RECEIVED</b> {web3utils.fromWei(data.value)} ONE{" "}
          ...
          <div className="">
            From {data.from}
            <span className="float-right">
              Status: {Number(data.returnData.success)}
            </span>
          </div>
        </div>
      </div>
  ...
  if (data.function.name == "makeTransfer") {
    return (
      <div className="mb-2">
        <div>
          <div>
            {data.function.name} to {data.function.params.to}{" "}
            <span className="float-right">
              {formatDate(new Date(data.timestamp * 1000))}
            </span>
          </div>
          <div className="">
            Status: {Number(data.returnData.success)} ({data.returnData.error})
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="mb-3">
        <div>
          <div>
            <b>{data.function.map((e) => formatFunction(e)).join(" -> ")}</b>{" "}
            <span className="float-right">
              {formatDate(new Date(data.timestamp * 1000))}
            </span>
          </div>
          <div className="">
            {desc}&nbsp;
            <span className="float-right">
              <small>Status: {Number(data.returnData.success)}</small>
            </span>
          </div>
        </div>
      </div>
            ...
            this.props.data.map((e) => {
              return (
                <TransactionRow
                  data={e}
                  key={e.hash}
                  harmonyClient={this.context.smartvault.harmonyClient}
