...
class SendPayment extends Component {
  ...
  componentDidMount() {
    ...
    this.context.smartvault.harmonyClient
      .getERC20Info(this.props.match.params.address)
      .then((e) => {
        self.setState({ ...e });
      });
  ...
  transfer(e) {
    ...
    const data = erc20.methods
      .transfer(
        fromBech32(this.state.destination),
        web3utils.toWei(this.state.sendAmount)
      )
      .encodeABI();
    const tx = RelayerClient.getContract()
      .methods.multiCall([
        { to: this.state.contractAddress, value: 0, data: data },
      ])
      .encodeABI();
    this.context.smartvault
      .submitTransaction(
        tx,
        parseInt(web3utils.toWei(this.state.gasPrice, "gwei")),
        this.state.gasLimit
      )
      .then((e) => {
        console.log("sigs", e);
