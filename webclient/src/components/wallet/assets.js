...
class AssetPage extends Component {
  ...
  loadHistory() {
    ...
    let erc20 = this.context.smartvault.walletData.erc20
      ? this.context.smartvault.walletData.erc20.slice()
      : [];
    this.context.smartvault.harmonyClient
      .getErc20Balance(
        erc20.filter((e) => e.contractAddress).map((e) => e.contractAddress),
              ...
              <div className="bg-light text-dark p-3 mb-2">
                ...
                <div className="float-right">
                  <span className="mr-4">
                    {this.state.balance &&
                      Number(
                        web3utils.fromWei(this.state.balance + "")
                      ).toFixedNoRounding(4)}{" "}
                    ONE
                  </span>
                  <Link to="/wallet/send_one">Send</Link> |{" "}
                  <Link to={"/wallet/viper?token=ONE"}>Swap</Link>
                </div>
              </div>
              {this.state.erc20 &&
                this.state.erc20.map((token, index) => {
                  return (
                    <div
                      className="bg-light text-dark p-3 mb-2"
                      key={token.name}
                    >
                      <StyledHoverBox>
                        <span>
                          ...
                          <a
                            href="#"
                            className="ml-1"
                            onClick={(e) => this.removeToken(e, index)}
                          >
                      ...
                      <div className="float-right">
                        <span className="mr-4">
                          {token.balance &&
                            Number(
                              web3utils.fromWei(token.balance + "")
                            ).toFixedNoRounding(4)}{" "}
                          {token.symbol}
                        </span>
                        <Link
                          to={"/wallet/send_hrc20/" + token.contractAddress}
                        >
                          Send
                        </Link>{" "}
                        |{" "}
                        <Link
                          to={"/wallet/viper?token=" + token.contractAddress}
                        >
                          Swap
                        </Link>
                      </div>
                        ...
                        <div className="form-group">
                          ...
                          <div className="">
                            <input
                              ...
                              onChange={this.updateContractAddress.bind(this)}
                            />
                          </div>
                        </div>
                        <div className="form-group">
