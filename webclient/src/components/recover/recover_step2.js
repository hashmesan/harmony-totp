...
class ProvideCode extends Component {
  ...
  componentDidMount() {
    ...
    this.context.smartvault.harmonyClient
      .isNameAvailable(this.state.name, 31536000)
      .then((available) => {
  ...
  recoverWallet() {
    ...
    this.context.smartvault
      .recoverWallet(this.state.name, codes, (status) => {
      ...
      <SmartVaultConsumer>
        {({ smartvault }) => (
          <React.Fragment>
            <h2>Provide 5-OTP Recovery Codes</h2>
            <h5 className="mt-4">
              Open your Google Authenticator. Type in code you see then click
              refresh to get a new code. Repeat until you completed 5 codes.
            </h5>
            <div className="justify-content-md-center">
              ...
              {[0, 1, 2, 3, 4].map((e) => {
                return (
                  <StyledOTPContainer className="row bg-white justify-content-md-center mb-4">
              ...
              {this.state.busy && (
                <button className="mt-5 btn btn-lg btn-secondary">
                  <span
                    className="spinner-border spinner-border-sm"
                    role="status"
                    aria-hidden="true"
